...
if (isset($_POST['submit'])) {
    ...
    if (!isset($error)) {
        ...
        $stmt->execute(array(':email' => $_POST['email']));
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        echo $token = hash_hmac('SHA256', $user->generate_entropy(8), $row['password']);//Hash and Key the random data
        echo "<br>" .   $storedToken = hash('SHA256', ($token));//Hash the key stored in the database, the normal value is sent to the user
        try {
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-8 col-md-6 offset-sm-2 offset-md-3 mt-3 mb-4">
            <form role="form" method="post" action="" autocomplete="off">
                ...
                <hr>
                <div class="row">
                    <div class="col-xs-12 col-md-12"><input type="submit" name="submit" value="Reset Password"
                                                            class="btn btn-lg btn-block btn-primary" tabindex="2"></div>
                </div>
            </form>
        </div>
    </div>
